...
void SwapInscriptionBuilder::CheckOrdSwapSig() const
{
    ...
    if (mSwapTx) {
        VerifyTxSignature(m_ord_input->output->Destination()->Address(), *mSwapTx, 0, move(spent_outs));
    }
    else {
        CMutableTransaction swap_tx(MakeSwapTx(has_funds_sig));
        VerifyTxSignature(m_ord_input->output->Destination()->Address(), swap_tx, 0, move(spent_outs));
    }
}
...
void SwapInscriptionBuilder::CheckFundsCommitSig() const
{
    ...
    if (mFundsCommitTx) {
        for (const auto& in: m_fund_inputs) {
            VerifyTxSignature(in.output->Destination()->Address(), *mFundsCommitTx, in.nin, std::vector<CTxOut>(spent_outs));
        }
    }
    else {
        CMutableTransaction swap_tx(MakeFundsCommitTx());
        for (const auto& in: m_fund_inputs) {
            VerifyTxSignature(in.output->Destination()->Address(), swap_tx, in.nin, std::vector<CTxOut>(spent_outs));
        }
    }
}
