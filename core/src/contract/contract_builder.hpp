...
public:
    ...
    CScript DummyScriptSig() const override { throw std::logic_error("P2Address abstract call"); }
...
public:
    ...
    P2PKH(ChainMode m, CAmount amount, std::string addr) : P2Address(m, amount, move(addr)) {}
    CScript PubKeyScript() const override;
...
public:
    ...
    void VerifyTxSignature(const std::string& addr, const CMutableTransaction& tx, uint32_t nin, std::vector<CTxOut> spent_outputs) const;
