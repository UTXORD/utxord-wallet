...
TEST_CASE("singleinout")
{
    ...
    ChannelKeys utxo_key = master_key.Derive("m/86'/1'/0'/0/66423");
    ...
    REQUIRE_NOTHROW(tx_contract.AddDestination(std::make_shared<TapRootKeyPath>(7000, destination_pk)));

    REQUIRE_NOTHROW(tx_contract.Sign(master_key));

    stringvector txs;
    REQUIRE_NOTHROW(txs = tx_contract.RawTransactions());
