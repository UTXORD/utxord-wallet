...
(async () => {
    try{
        ...
        */
        ...
        self.index = 1
        self.lastListenIndex = 1;

        function timeToListen() {
            if( (self.index - self.lastListenIndex) > 9  || self.index === 1 )
            {
                self.lastListenIndex = self.index;
                return true;
            }
        }
        ...
        chrome.alarms.onAlarm.addListener(async () => {
            console.log('index:',self.index, '| lastListenIndex:',self.lastListenIndex);
            if(timeToListen()){
                await listener();
            }
